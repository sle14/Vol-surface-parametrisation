...
template,transparent = "plotly_white","rgba(0,0,0,0)"
...
def plot2d_layout_subplot(fig,x1_title,x2_title,y1_title,y2_title):
    fig.update_layout(template=template,paper_bgcolor=transparent,plot_bgcolor=transparent)
...
def plot3d_boundaries(df,num):
    ...
    for K in dfc["Strike"].unique():
        try:
            ...
            boundaries = dfc[dfc["Strike"]==K]["eebC"].iloc[0]
    ...
    for K in dfc["Strike"].unique():
        try:
            ...
            boundaries = dfc[dfc["Strike"]==K]["eebP"].iloc[0]
        z = [float(i) for i in boundaries]
        dt = tenors[num]/len(boundaries)
        y = np.flip(np.array([i*dt for i in range(len(boundaries))]))
        x = [K]*len(boundaries)
...
def plot3d_single(fig,df,S_col,x_col,z_col,atm_plots=False,div_plot=False,payoff_plot=False,params_plot=False,idx=None):
    ...
    for t in df["Tenor"].unique():
        ...
        fig = trace_3d(fig,dfx[x_col],T,dfx[z_col],f"{t} Quoted {z_col}","markers","red",2,idx=idx,group="z_col") #Quoted px,vol,var,totvar
        if payoff_plot != False:
            ...
            if x_col == "Moneyness":
                ...
                x = np.log(x/F)
            fig = trace_3d(fig,x,y,z,f"{t} {R} Payoff","lines","darkblue",idx=idx,group="Payoffs")
        if params_plot != False:
            ...
            try:
                ...
                params = dfx["SurfaceParams"].iloc[0]
            w = optimiser.SSVI().surface(np.array(k),dfx["TotAtmVar"].iloc[0],*params)
            if "Vol" in z_col:
                w = np.sqrt(w/(t/365)) 
            elif "Var" in z_col and not "Tot" in z_col:
                w = w/(t/365)
            fig = trace_3d(fig,dfx[x_col],T,w,f"{t} Fitted {z_col}","lines","darkred",idx=idx,group="Fitted") #Fitted
        if atm_plots != False:
            if x_col == "Moneyness":
                x0,x1 = 1,F/S
            elif x_col == "LogStrike":
...
def plot2d_quotes(df,T,x,y1,y2):
    fig = make_subplots(rows=1,cols=2)
    for t in df[T].unique():
        dfx = df[df[T]==t]
        fig = trace_2d(fig,dfx[x],dfx[y1],f"{t} {y1}",[1,1])
        fig = trace_2d(fig,dfx[x],dfx[y2],f"{t} {y2}",[1,2])
    fig = plot2d_layout_subplot(fig,x,x,y1,y2)
    fig.show()    
...
def plot2d_fitted(df,V,T,F,a,b,m):
    try:
        ...
        params = df["SurfaceParams"].iloc[0]
    K = np.linspace(a,b,m)
    fig = make_subplots(rows=1,cols=2)
